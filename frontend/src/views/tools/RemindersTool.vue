...
async function createReminder() {
  ...
  try {
    ...
    const response = await secretaryStore.manageReminder(message)
...
async function listReminders() {
  ...
  try {
    const response = await secretaryStore.manageReminder('显示我的提醒列表')
    result.value = response
  } catch (err: any) {
    error.value = err.message || 'Failed to list reminders'
  } finally {
    loading.value = false
  }
}
...
async function listUpcomingReminders() {
  ...
  try {
    const response = await secretaryStore.manageReminder('显示今天的提醒')
    result.value = response
  } catch (err: any) {
    error.value = err.message || 'Failed to get upcoming reminders'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
      ...
      <p class="text-gray-600 mb-6">Set and manage your reminders.</p>
